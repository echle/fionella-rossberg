...
- [ ] T001 Create Vite project with TypeScript template in project root
- [ ] T002 Install core dependencies: phaser@3.80+, zustand@4.x in package.json
- [ ] T003 [P] Install dev dependencies: vitest, @vitest/ui, vite-plugin-pwa in package.json
- [ ] T004 [P] Install tooling: eslint, prettier, @typescript-eslint packages in package.json
- [ ] T005 Configure TypeScript strict mode in tsconfig.json
- [ ] T006 Configure Vite build settings and PWA plugin in vite.config.ts
- [ ] T007 Configure Vitest with jsdom environment in vitest.config.ts
- [ ] T008 [P] Setup ESLint rules in .eslintrc.js
- [ ] T009 [P] Setup Prettier config in .prettierrc
- [ ] T010 Create folder structure: src/{config,scenes,entities,state,systems,utils}
- [ ] T011 [P] Create folder structure: assets/{sprites,ui,backgrounds}
- [ ] T012 [P] Create folder structure: tests/{unit,integration}
- [ ] T013 Add npm scripts in package.json: dev, build, test, lint, format
- [ ] T014 Create public/index.html with canvas container element
- [ ] T015 [P] Create public/manifest.json for PWA configuration
...
- [ ] T016 Define TypeScript interfaces in src/state/types.ts: GameState, HorseStatus, Inventory, UIState
- [ ] T017 Create Phaser game configuration in src/config/phaserConfig.ts with canvas, scale, physics settings
- [ ] T018 Create game constants in src/config/gameConstants.ts: decay rates, status increments, initial values
- [ ] T019 Initialize Zustand store in src/state/gameStore.ts with default state structure
- [ ] T020 Create main entry point in src/main.ts: Initialize Phaser game instance
- [ ] T021 Create BootScene in src/scenes/BootScene.ts for asset preloading
- [ ] T022 Create MainGameScene in src/scenes/MainGameScene.ts with empty create() method
- [ ] T023 Create UIScene in src/scenes/UIScene.ts with empty create() method
- [ ] T024 [P] Create utility functions in src/utils/mathUtils.ts: clamp, lerp
- [ ] T025 [P] Create utility functions in src/utils/timeUtils.ts: elapsed time calculations
- [ ] T026 Verify dev server runs successfully with `npm run dev`
...
- [ ] T034 [US1] Render Hunger status bar in UIScene.ts create() method, position top-left
- [ ] T035 [P] [US1] Render Cleanliness status bar in UIScene.ts, position top-center
- [ ] T036 [P] [US1] Render Happiness status bar in UIScene.ts, position top-right
- [ ] T037 [US1] Bind status bar values to gameStore.horse in UIScene.ts update() loop
- [ ] T038 [US1] Implement responsive scaling in phaserConfig.ts Scale Manager config
- [ ] T039 [US1] Test on desktop (1920x1080) and mobile (375x667) viewports manually
- [ ] T040 [US1] Write unit test in tests/unit/gameStore.test.ts: verify initial state values

**US1 Complete**: Horse and status bars visible, responsive
...
- [ ] T055 [US2] Create AnimationSystem in src/systems/AnimationSystem.ts: listen for animation events
- [ ] T056 [US2] Gray out carrot icon when carrots === 0 in UIScene.ts render logic
- [ ] T057 [US2] Prevent selection when carrots === 0 in selectTool action validation
- [ ] T058 [US2] Write unit test in tests/unit/actions.test.ts: feed() increases hunger, consumes carrot
- [ ] T059 [US2] Write unit test in tests/unit/actions.test.ts: feed() clamps hunger at 100
- [ ] T060 [US2] Write unit test in tests/unit/actions.test.ts: feed() fails when carrots === 0
...
- [ ] T073 [US3] Call groom() per drag stroke in InputSystem.ts event handler
- [ ] T074 [US3] Create particle emitter in MainGameScene.ts: spawn sparkles at drag position
- [ ] T075 [US3] Trigger sparkle particles during grooming in MainGameScene.ts based on events
- [ ] T076 [US3] Gray out brush icon when brushUses === 0 in UIScene.ts render logic
- [ ] T077 [US3] Prevent selection when brushUses === 0 in selectTool action validation
- [ ] T078 [US3] Write unit test in tests/unit/actions.test.ts: groom() increases cleanliness, consumes brush use
- [ ] T079 [US3] Write unit test in tests/unit/actions.test.ts: groom() clamps cleanliness at 100
- [ ] T080 [US3] Write unit test in tests/unit/actions.test.ts: groom() fails when brushUses === 0
...
- [ ] T088 [US4] Create heart particle emitter in MainGameScene.ts: spawn hearts above horse
- [ ] T089 [US4] Trigger heart particles during petting in MainGameScene.ts based on events
- [ ] T090 [US4] Write unit test in tests/unit/actions.test.ts: pet() increases happiness by 10
- [ ] T091 [US4] Write unit test in tests/unit/actions.test.ts: pet() clamps happiness at 100
- [ ] T092 [US4] Write unit test in tests/unit/actions.test.ts: pet() never fails (always available)
...
- [ ] T093 [US5] Implement applyDecay action in src/state/actions.ts: calculate decay based on elapsed ms
- [ ] T094 [US5] Apply decay rates from gameConstants.ts: hunger -1/6s, cleanliness -1/12s, happiness -1/7.5s
- [ ] T095 [US5] Clamp all status values at minimum 0 in applyDecay logic
- [ ] T096 [US5] Create DecaySystem in src/systems/DecaySystem.ts: call applyDecay() every update tick
- [ ] T097 [US5] Initialize DecaySystem in MainGameScene.ts create() method
- [ ] T098 [US5] Update DecaySystem in MainGameScene.ts update() loop: pass delta time to applyDecay
- [ ] T099 [US5] Write unit test in tests/unit/DecaySystem.test.ts: verify decay calculations for 60s elapsed
- [ ] T100 [US5] Write unit test in tests/unit/DecaySystem.test.ts: verify values clamp at 0 (no negatives)
- [ ] T101 [US5] Write unit test in tests/unit/DecaySystem.test.ts: verify different decay rates per status
...
- [ ] T102 Create SaveSystem in src/systems/SaveSystem.ts with save() and load() methods
- [ ] T103 Implement save() in SaveSystem.ts: serialize gameState to JSON, write to LocalStorage key
- [ ] T104 Implement load() in SaveSystem.ts: read from LocalStorage, parse JSON, validate schema
- [ ] T105 Calculate elapsed time in load() method: Date.now() - savedState.timestamp
- [ ] T106 Apply decay for elapsed time in load() method: call applyDecay(elapsedMs) before restoring state
- [ ] T107 Implement loadGameState action in src/state/actions.ts: merge loaded state into store
- [ ] T108 Call SaveSystem.load() in BootScene.ts create() method before launching MainGameScene
- [ ] T109 Trigger auto-save every 10 seconds in MainGameScene.ts: setInterval calling SaveSystem.save()
- [ ] T110 Trigger save after each interaction in actions.ts: feed(), groom(), pet() call SaveSystem.save()
- [ ] T111 Add window.onbeforeunload handler in main.ts: save on tab close/refresh
- [ ] T112 Write unit test in tests/unit/SaveSystem.test.ts: verify save serialization matches schema
- [ ] T113 Write unit test in tests/unit/SaveSystem.test.ts: verify load deserialization and elapsed time
- [ ] T114 Write unit test in tests/unit/SaveSystem.test.ts: verify corrupted data fallback to new game
...
- [ ] T121 [P] Add touch-friendly hit boxes on Horse sprite in MainGameScene.ts: 20% larger than visual
- [ ] T122 [P] Implement input debouncing in InputSystem.ts: prevent spam clicks (100ms cooldown)
- [ ] T123 [P] Optimize texture atlas generation for all sprites using TexturePacker or Shoebox
- [ ] T124 [P] Test responsive scaling on mobile devices: iPhone SE (375x667), iPad (768x1024)
- [ ] T125 [P] Test responsive scaling on desktop: 1920x1080, 2560x1440
- [ ] T126 [P] Profile performance in Chrome DevTools: verify 60 FPS maintained
- [ ] T127 [P] Run ESLint and fix all warnings: `npm run lint`
- [ ] T128 [P] Run Prettier and format all code: `npm run format`
- [ ] T129 [P] Run full test suite with coverage: `npm run test:coverage` (verify >= 70%)
- [ ] T130 [P] Test cross-browser manually: Chrome, Firefox, Safari, Edge
- [ ] T131 [P] Fix any console errors or warnings in production build
- [ ] T132 Build production bundle: `npm run build` and verify dist/ output
